...
// @ts-ignore
const hasZodShape = (x: unknown): x is { shape: Record<string, any> } => !!x.shape;
...
const flattenParser = (parser: z.AnyZodObject): ParserPaths => {
  let flat: ParserPaths = new Map(); 

  if (hasZodShape(parser)) {
    for (const key in parser.shape) {
      const subFlat = flattenParser(parser.shape[key]);
      
      for (const [subPath, parser] of subFlat.entries()) {
        const path = [key, ...subPath];
        flat.set(path, parser);
      }
    }
  } else {
    flat.set([], parser);
...
const defaultEnvironmentKernel = (parser: unknown): unknown => {
  if (hasZodShape(parser)) {
    return Object.fromEntries(Object.entries(parser.shape).map(([key, value]) => ([key, defaultEnvironment(value)])))
  }
  return undefined;
} 

const defaultEnvironment = (parser: z.AnyZodObject): Record<string, unknown> => defaultEnvironmentKernel(parser) as Record<string, unknown>;
 ...
 * @returns 
 */
export const environmentLoader = <T extends z.AnyZodObject>(parser: T, renameVars?: Record<KeysToAllCapsCase<T['shape']>, string>): IncrementalConfigLoader => {
  const load = () => {
    let environment = defaultEnvironment(parser);

    const flat = flattenParser(parser);

    for (const [path, parser] of flat.entries()) {
      // This is the default formatting for our paths
      // WARNING: Notice the type assertion below
      const defaultName = path.map(x => x.toUpperCase()).join('_') as KeysToAllCapsCase<T['shape']>;
      ...
      const parsed = parser.optional().safeParse(value);
      if (!parsed.success) {
        throw new ErrorWithContext('Issue sourcing config from environment', { variableName, value, issues: parsed.error.issues });
      }
